package com.pengcoder.mock;

import com.alibaba.fastjson.JSON;
import com.alibaba.fastjson.JSONObject;
import com.pengcoder.mock.util.LogUpLoader;
import com.pengcoder.mock.util.RanOpt;
import com.pengcoder.mock.util.RandomNum;
import com.pengcoder.mock.util.RandomOptionGroup;
...
//    RanOpt[] opts = {new RanOpt()}
    ...
    String appId = "pengcoder";

    public String EventLog(String startuplog) {
        ...
        JSONObject startLog = JSON.parseObject(startuplog);
        ...
        String evid = eventOptionGroup.getRanOpt().getValue();
    ...
    public String StartUpLog() {
     ...
     `area` string COMMENT '城市' */
        ...
        String mid = "mid_" + RandomNum.getRandomInt(1, 2500);
        String uid = "" + RandomNum.getRandomInt(1, 500);
        String os = osOptionGroup.getRanOpt().getValue();
        String appid = this.appId + "_" + RandomNum.getRandomInt(1, 100);
        String area = areaOptionGroup.getRanOpt().getValue();
        String vs = vsOptionGroup.getRanOpt().getValue();
        //long ts= logDateUtil.getRandomDate().getTime();
        String ch = os.equals("ios") ? "appstore" : channelOptionGroup.getRanOpt().getValue();
    ...
    public static void genLog() {
        int eventNum = 10000000;

        for (int i = 0; i < eventNum; i++) {
            JsonMock jsonMock = new JsonMock();
            String startuplog = jsonMock.StartUpLog();
            LogUpLoader.uplog(startuplog);
//            System.out.println(startuplog);
            while (!jsonMock.isQuitGroup.getRanOpt().getValue()) {
                String eventLog = jsonMock.EventLog(startuplog);
//                System.out.println(eventLog);
                LogUpLoader.uplog(eventLog);
            }
